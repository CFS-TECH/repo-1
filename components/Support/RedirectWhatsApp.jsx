...
const FloatingContact = ({ phoneNumber = "+916398149627" }) => {
  ...
  return (
    <div className="fixed flex flex-col gap-4 md:bottom-10 md:right-10 bottom-6 right-6 z-[100]">
      {/* --- SUPPORT / CALL BUTTON --- */}
      <motion.div
        initial="hidden"
        animate="visible"
      ...
      >
        ...
        <Link
          ...
          className="relative flex items-center justify-center w-14 h-14 bg-white border-2 border-[#fd741e] shadow-lg rounded-full z-10"
        >
          <BiSupport className="text-3xl text-[#fd741e]" />
        </Link>
      </motion.div>
      ...
      >
        ...
        <Link
          ...
          className="relative flex items-center justify-center w-14 h-14 bg-[#25D366] shadow-[0_4px_20px_rgba(37,211,102,0.4)] rounded-full z-10"
        >
          <RiWhatsappFill className="text-4xl text-white" />
        </Link>
      </motion.div>
    </div>
  );
